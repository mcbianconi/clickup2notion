...
async def get_task_attachments(session, task, semaphore):
    ...
    async with semaphore:
        async with session.get(url) as response:
            ...
            logger.info(f"Encontrados {len(clickup_task_data['attachments'])} anexos na tarefa {task['id']}")
...
async def create_notion_properties(notion_database_id, notion):
    ...
    properties_to_update = {key: value for key, value in new_properties.items() if key not in existing_properties}
...
async def create_notion_page(notion: AsyncClient, task, notion_database_id) -> None:
    ...
    if task['id'] in _attachments_store:
        ...
        attachments = [
            ...
            } for attachment in _attachments_store[task['id']] if not attachment['deleted']
        ]
...
async def export(clickup_list_id, notion_database_id, clickup_api_token, notion_api_token):
    ...
    async with CachedSession(headers=clickup_headers, cache=file_cache) as clickup_session:
        ...
        for task in tasks:
            get_attachment_tasks.append(get_task_attachments(clickup_session, task, attachment_semaphore))
            
            notion_task = clickup2notion(task)
            create_page_tasks.append(create_notion_page(notion, notion_task, notion_database_id))

        
        logger.info(f"Pegando anexos de {len(get_attachment_tasks)} tarefas com {attachment_semaphore._value} requisições simultâneas")
...
@click.option('--notion_database_id', required=True, help='ID da base de dados do Notion')
def main(clickup_list_id, notion_database_id):
    clickup_api_token = os.getenv('CLICKUP_API_TOKEN')
    notion_api_token = os.getenv('NOTION_API_TOKEN')

    if not clickup_api_token or not notion_api_token:
        click.echo('Os tokens de API não foram encontrados. Verifique seu arquivo .env.')
        return

    asyncio.run(export(clickup_list_id, notion_database_id, clickup_api_token, notion_api_token))
